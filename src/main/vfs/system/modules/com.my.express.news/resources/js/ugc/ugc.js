$(function () {
    ...
    function myMappings() {
        ...
        // values marked OPTIONAL will case the field to be removed in the XML
        // content if the form value is empty
        // UPLOAD is required for all form elements that are of type <input
        // type="file">
        ugc.map("title", "Title[1]");
        ugc.map("shortDescription", "ShortDescription[1]");
        ugc.map("thumbnail", "Thumbnail[1]", ugc.UPLOAD);
        ugc.map("videoLink", "VideoLink[1]");
        ugc.map("content", "Content[1]");
        ugc.map("categories", "Categories[1]");
        ugc.map("date", "Date[1]");
        ugc.map("comment", "Comment[1]");
        ugc.map("view", "View[1]");
        ugc.map("like", "Like[1]");
    ...
    function myErrorHandler(errorType, message, additionalData) {
        // very simple error handler that just displays an alert box with the
        // raw error text
    ...
    function myFormInitHandler() {
        ...
        // // purpose here: show the "remove image" option only if there actually
        // // IS an image in the existing content
    ...
    function myWaitHandler() {
        // // display a DIV in the HTML that contains a "please wait, loading"
        // // animation
        // if (isWaiting) {
    ...
    function updateValues(uploadResults) {
        ...
        if (uploads != null) {
            // we do have uploads
            var image = uploads["thumbnail"];
            if (image) {
                // an image was uploaded
                ugc.setContent("thumbnail", image);
            }
        }
        ...
        //
        // if (!ugc.contentHas("imagefile")
        //     || ugc.getForm("imageremove").prop('checked')) {
        //     // no image set or special form option "remove image" has been
        //     // checked
    ...
    function afterUploadHandler(uploadResult) {
        ...
        // images are already uploaded, now generate the XML content that refers
        // the images from all the form values
        ugc.saveContent(updateValues(uploadResult), afterSaveHandler);
    }

    function save() {
        // save the form - step 1 of 3
        // first the uploads must be handled, afterwards the form itself is
        // processed in a callback
        ugc.uploadFiles(["thumbnail"], afterUploadHandler);
    }

    function validate() {
        // validation of the form in this example relies on the server side, so
        // we need another callback handler
    ...
    function validateHandler(errors) {
        // form validation handler that displays the results of the server side
        // validation in a simple alert
